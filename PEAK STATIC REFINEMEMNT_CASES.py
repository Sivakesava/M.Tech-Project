#---------------------------------------------------------------#
#FOR DIFFERENT CASES THIS CODE STATTICALLY REFINES
#THE ALL ELEMENTS CONTAING GAUSSIAN CENTRE AT ANY TIME
#FIRST CALCULTES THE ELEMNTS THROUGH WHICH PEAK PASSES
#AND THEN REFINEMENT STARTS FROM THE 1ST TIME STEP.
...
#########################################################################
#########################################################################
print '-------------STATIC PEAK REFINEMENT FOR DIFFERENT CASES------------ \n'
Option=1
for case in range(5):
   if case==0:
      print'N=1'
...
##########Calculation of Error norms###############
      ...
      if case==0:
         L2_1[Step],Nop_1[Step],WCT_1[Step]=L2,Nop+n_sub*(Nop_m-Qn),WCT
      elif case==1:
         L2_2[Step],Nop_2[Step],WCT_2[Step]=L2,Nop+n_sub*(Nop_m-Qn),WCT
      elif case==2:
         L2_4[Step],Nop_4[Step],WCT_4[Step]=L2,Nop+n_sub*(Nop_m-Qn),WCT
      elif case==3:
         L2_8[Step],Nop_8[Step],WCT_8[Step]=L2,Nop+n_sub*(Nop_m-Qn),WCT
      else:#if case==4:
         L2_16[Step],Nop_16[Step],WCT_16[Step]=L2,Nop+n_sub*(Nop_m-Qn),WCT
print 'dt=', dt
import pylab as py
