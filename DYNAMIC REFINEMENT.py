#----------------------------------------------------------------#
#THIS CODE DYNAMICALLY REFINES PEAK OF THE GAUSSIAN WITHOUT AMR
...
def projection_matrices(z,w,zcal,x,y,Nel,n,Q):
    ...
    return MS1,MS2,MS1_2,MS2_2,Nopx_m,Nop_m,MMS1,MMS2,MMS3,MMS4
########################################################################
########################################################################
             ...
             for k in range(n):
                if (i==2 or i==3):
                    ...
                    r_flux_sub[index_][i][0][k]=0.5*(n1*(fv_sub[index_][i][0][k]+fv_sub[index_][i-2][n-1][k])-
                                                     lamb*(q_sub[index_][i-2][n-1][k]-q_sub[index_][i][0][k]))
                if (i==0 or i==2):
                    ...
                    r_flux_sub[index_][i][1][k]=0.5*(n2*(fu_sub[index_][i][k][n-1]+fu_sub[index_][i+1][k][0])-
                                                     lamb*(q_sub[index_][i+1][k][0]-q_sub[index_][i][k][n-1]))
                if (i==0 or i==1):
                    ...
                    r_flux_sub[index_][i][2][k]=0.5*(n3*(fv_sub[index_][i][n-1][k]+fv_sub[index_][i+2][0][k])-
                                                     lamb*(q_sub[index_][i+2][0][k]-q_sub[index_][i][n-1][k]))
                if (i==1 or i==3):
                    ...
                    r_flux_sub[index_][i][3][k]=0.5*(n4*(fu_sub[index_][i][k][0]+fu_sub[index_][i-1][k][n-1])-
                                                     lamb*(q_sub[index_][i-1][k][n-1]-q_sub[index_][i][k][0]))
...
def L_Norm_Image(x,y,x_sub,y_sub,n,Nel,Nop,Nopx,Nop_m,Nopx_m,Nelxy,T_step,qn,qn_sub):
    ...
    for i in range(n_sub):###Refined elements
       ...
       surf=ax.plot_surface(xnp,ynp,q_num,rstride=1,cstride=1,cmap=cm.coolwarm, linewidth=0, antialiased=False)
    py.title('q_numerical(t)(x,y)')##tirle of plots
    py.xlabel('x')
    py.ylabel('y')
    cb = fig.colorbar(surf)
    it_no=(Time+1)/10###Saving each figure in PNG format
    fileName = "/home/sivakesava/DR_Fig0416/image-%04d.png" %it_no
...
#-----------------------------------------------------------------------#
#########################################################################
print '-------------DYNAMIC REFINEMENT OF GAUSSIAN WAVE------------ \n'
...
[MS1,MS2,MS1_2,MS2_2,Nopx_m,Nop_m,MMS1,MMS2,MMS3,MMS4]=projection_matrices(z,w,zcal,x,y,Nel,n,Q);
#---------------######################################################################--------------#
...
xc,yc,T_step,Ck=-0.5,0.0,0.0,0
#####----From here problem will be solved for T time stpes
p=subprocess.call(['mkdir', '/home/sivakesava/DR_Fig0416'])# Creating a folder to save the images
for Time in range(T):
    T_step=T_step+dt
        ...
        for ELM in range(Nelxy):    
            if c_status[ELM]==1:# Checking current status of each element
                if r_status[ELM]==0: # checking refinement status of element
                    n_sub=n_sub+1# No. of elements to be refined after this time step
                    ...
                    uo_sub,vo_sub=u_sub,v_sub
                    xx_s,yy_s,xx_l,yy_l=x_sub,y_sub,x_lsub,y_lsub
                    r_status[ELM]=1
    ...
    if ((Time+1)%10)==0:
       L2=L_Norm_Image(x,y,x_sub,y_sub,n,Nel,Nop,Nopx,Nop_m,Nopx_m,Nelxy,T_step,qn,qn_sub);
       Ck=int((Time+1)/10)-1
...
for ELM in range(Nelxy):# Normal elements
   if r_status[ELM]==0:
      surf=ax.plot_surface(x[ELM],y[ELM],qn[ELM],rstride=1,cstride=1,cmap=cm.RdBu,linewidth=0, antialiased=False)
for i in range(n_sub):###Refined elements
   x_p,y_p=x_lsub[i].reshape(2*n,1),y_lsub[i].reshape(2*n,1)
   xnp,ynp=numpy.meshgrid(x_p,y_p)
   q_num=Reshape_(qn_sub[i],n,2,4,2*n);
...
py.title('L2_Norm vs Time, Ne=16*16, N=4')
py.xlabel('Time(sec)')
py.ylabel('L2_Norm')
fileName = "/home/sivakesava/L2Norm.png"
plt.savefig(fileName, format="png")
#py.show()
########### Making animation using saved png images##########################
pp=subprocess.call(['ffmpeg', '-f', 'image2', '-r', '30','-i',
                    'DR_Fig0416/image-%04d.png', '-vcodec', 'mpeg4', '-y', 'DR_GaussianMotion.mp4'])
subprocess.Popen("vlc DR_GaussianMotion.mp4", shell=True)
#################################################################
#---------------------END OF THE CODE---------------------------#
#################################################################
